...
class MathTest:
    def __init__(self, root, username, callback=None):
        ...
        self.top_frame.pack(fill=tk.BOTH, padx=15, pady=10)

        self.time_label = tk.Label(self.top_frame, font=("Open Sans", 24), text=f"Time: {self.remaining_time}", anchor='w',
                                  bg="white", fg="#3a3d42")
        self.time_label.pack(side=tk.LEFT)

        self.question_label = tk.Label(self.root, font=("Open Sans", 140), bg="white", fg="#3a3d42")
        ...
        style.configure('Black.TButton',  # Custom style name
                        font=('Open Sans', 40),
                        background='black',  # Set background to black
                        foreground='white',  # Set text color to white
                        borderwidth=0,  # Remove border
                        relief='flat')  # Remove button relief (3D effect)

        # Map the style to remove any default borders or separators
        style.map('Black.TButton',
                  background=[('active', 'black'), ('pressed', 'black')],  # Keep background black on hover and click
                  foreground=[('active', 'white'), ('pressed', 'white')])  # Keep text white on hover and click
        ...
        for i in range(4):
            btn = ttk.Button(self.buttons_frame, text="", style='Black.TButton')  # Apply the custom style
